...
const before = await page.evaluate(()=>{ const p=TEKERLEK.game.player; p.velocity.set(0,0,22); return [p.position.x,p.position.z,p.heading]; });
await page.keyboard.down('KeyD');
await new Promise(r=>setTimeout(r,2500));
await page.keyboard.up('KeyD');
const after = await page.evaluate(()=>{ const p=TEKERLEK.game.player; return [p.position.x,p.position.z,p.heading]; });
...
console.log('paused:', await page.evaluate(()=>TEKERLEK.game.loop.paused));
...
console.log('sun intensity after worldLight=2:', await page.evaluate(()=>TEKERLEK.game.renderer.sun.intensity.toFixed(2)));
console.log('uGain after brightness=1.4:', await page.evaluate(()=>TEKERLEK.game.renderer.postfx.uniforms.uGain.value.toFixed(3)));
